...
from model.emb_methods.s2v.alias import create_alias_table
from model.emb_methods.s2v.utils import partition_dict, preprocess_nxgraph
from model.emb_methods.s2v.walker import BiasedWalker


class Struc2Vec():
    def __init__(self, graph, walk_length=6, num_walks=20, workers=1, verbose=0, stay_prob=0.3, opt1_reduce_len=True,
    ...
    def train(self, embed_size=32, window_size=5, workers=3, iter=5, seed=42):
        sentences = self.sentences

        model = Word2Vec(sentences, vector_size=embed_size, window=window_size, min_count=1, hs=1, sg=1,
                         workers=workers,
                         epochs=iter,
                         seed=seed)
